...
pipeline {
    ...
    stages {
        stage('Start') {
            steps {
                ...
                notifySlack('STARTED', CHANNEL, "$env.BRANCH_NAME", COMMIT_MESSAGE, AUTHOR)
            }
        }
    ...
    post {
        success {
            notifySlack('SUCCESS', CHANNEL, "$env.BRANCH_NAME", COMMIT_MESSAGE, AUTHOR)
        }
        failure {
            notifySlack('FAILURE', CHANNEL, "$env.BRANCH_NAME", COMMIT_MESSAGE, AUTHOR)
        }
        always {
            cleanWs()
        }
